...
func NewAuthMiddleware(credentialFile string, unAuthorized func(c *gin.Context)) (*FirebaseAuthMiddleware, error) {
...
func (fam *FirebaseAuthMiddleware) AuthenticationFunc() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		idToken, err := fam.cli.VerifyIDToken(c, token)
		if err != nil {
			if fam.unAuthorized != nil {
				fam.unAuthorized(c)
			} else {
		...
		//set a value (firebase token) to the gin context (via the name "FIREBASE_ID_TOKEN")
		log.Println(idToken)
