...
// forTest = true: Do not write logs to files (develop environment)
// forTest = false: Write logs to files (production environment)
func InitLogger(forTest bool) *MyLogger {
	...
	if !forTest {
		writerInfo, err := rotatelogs.New(
			"./logs/info/backend_go_%Y%m%d_info.log",
		...
		writerError, err := rotatelogs.New(
			"./logs/error/backend_go_%Y%m%d_error.log",
			rotatelogs.WithMaxAge(30*24*time.Hour),
			rotatelogs.WithRotationTime(24*time.Hour),
		)
		if err != nil {
